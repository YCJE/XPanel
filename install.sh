...
setup_ssl_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}Please ensure port 80 is open and try again later with: xpanel${plain}"
...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload xpanel before expiry.${plain}"
    return 0
}
...
ssl_cert_issue() {
    ...
    echo -e "${green}Default --reloadcmd for ACME is: ${yellow}systemctl restart xpanel || rc-service xpanel restart${plain}"
    echo -e "${green}This command will run on every certificate issue and renew.${plain}"
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; systemctl restart xpanel"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                echo -e "${green}Reloadcmd is: systemctl reload nginx ; systemctl restart xpanel${plain}"
                reloadCmd="systemctl reload nginx ; systemctl restart xpanel"
                ;;
            2)
                echo -e "${yellow}It's recommended to put xpanel restart at the end${plain}"
...
install_xpanel() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$tag_version" ]]; then
            ...
            if [[ ! -n "$tag_version" ]]; then
                echo -e "${red}Failed to fetch xpanel version, it may be due to GitHub API restrictions, please try it later${plain}"
                exit 1
            fi
        fi
        echo -e "Got xpanel latest version: ${tag_version}, beginning the installation..."
        curl -4fLRo ${xpanel_folder}-linux-$(arch).tar.gz https://github.com/YCJE/XPanel/releases/download/${tag_version}/xpanel-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading xpanel failed, please be sure that your server can access GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        echo -e "Beginning to install xpanel $1"
        curl -4fLRo ${xpanel_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download xpanel $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    curl -4fLRo /usr/bin/xpanel-temp https://raw.githubusercontent.com/YCJE/XPanel/main/xpanel.sh
    if [[ $? -ne 0 ]]; then
        echo -e "${red}Failed to download xpanel.sh${plain}"
        exit 1
    fi
    ...
    tar zxvf xpanel-linux-$(arch).tar.gz
    ...
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "xpanel/xpanel.db" "/etc/.gitignore"; then
                ...
                echo -e "${green}Added xpanel.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "xpanel/xpanel.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added xpanel.db for etckeeper${plain}"
        fi
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download xpanel.rc${plain}"
            exit 1
        fi
        ...
        if [ -f "xpanel.service" ]; then
            echo -e "${green}Found xpanel.service in extracted files, installing...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "xpanel.service.debian" ]; then
                        echo -e "${green}Found xpanel.service.debian in extracted files, installing...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "xpanel.service.arch" ]; then
                        echo -e "${green}Found xpanel.service.arch in extracted files, installing...${plain}"
                ...
                *)
                    if [ -f "xpanel.service.rhel" ]; then
                        echo -e "${green}Found xpanel.service.rhel in extracted files, installing...${plain}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            if [[ $? -ne 0 ]]; then
                echo -e "${red}Failed to install xpanel.service from GitHub${plain}"
                exit 1
            fi
            service_installed=true
        fi
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}Failed to install xpanel.service file${plain}"
            exit 1
        fi
    fi

    echo -e "${green}xpanel ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
