...
log_folder="${XPANEL_LOG_FOLDER:=/var/log/xpanel}"
mkdir -p "${log_folder}"
...
update() {
    confirm "This function will update all xpanel components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
update_menu() {
    ...
    curl -fLRo /usr/bin/xpanel https://raw.githubusercontent.com/YCJE/XPanel/main/xpanel.sh
    chmod +x ${xpanel_folder}/xpanel.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/YCJE/XPanel/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del xpanel
        rm /etc/init.d/xpanel -f
    else
        systemctl stop xpanel
        systemctl disable xpanel
        rm ${xpanel_service}/xpanel.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/YCJE/XPanel/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xpanel_folder}/xpanel setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${xpanel_folder}/xpanel setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the XPanel panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${xpanel_folder}/xpanel setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${xpanel_folder}/xpanel setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xpanel_folder}/xpanel setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xpanel_folder}/xpanel setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "xpanel Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "xpanel and xray stopped successfully"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "xpanel and xray Restarted successfully"
    else
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service xpanel status
    else
        systemctl status xpanel -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add xpanel default
    else
        systemctl enable xpanel
    fi
    if [[ $? == 0 ]]; then
        LOGI "xpanel Set to boot automatically on startup successfully"
    else
        LOGE "xpanel Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "xpanel Autostart Cancelled successfully"
    else
        LOGE "xpanel Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                grep -F 'xpanel[' /var/log/messages
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u xpanel -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-xpanel.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-xpanel.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-xpanel.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-xpanel.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/xpanel -z /usr/bin/xpanel https://github.com/YCJE/XPanel/raw/main/xpanel.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        if [[ ! -f /etc/init.d/xpanel ]]; then
            return 2
        fi
        if [[ $(rc-service xpanel status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        if [[ ! -f ${xpanel_service}/xpanel.service ]]; then
            return 2
        fi
        temp=$(systemctl status xpanel | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'xpanel' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled xpanel)
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        curl -fLRo ${xpanel_folder}/bin/${dat}.dat -z ${xpanel_folder}/bin/${dat}.dat \
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xpanel_folder}/xpanel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
...
ssl_cert_issue() {
    ...
    reloadCmd="xpanel restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}xpanel restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; xpanel restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; xpanel restart"
                reloadCmd="systemctl reload nginx ; xpanel restart"
                ;;
            2)
                LOGD "It's recommended to put xpanel restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; xpanel restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xpanel_folder}/xpanel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xpanel_folder}/xpanel setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xpanel_folder}/xpanel setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xpanel_folder}/xpanel setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xpanel_folder}/xpanel setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${xpanel_folder}/xpanel setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${xpanel_folder}/xpanel setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
show_usage() {
    echo -e "┌────────────────────────────────────────────────────────────────┐
│  ${blue}xpanel control menu usages (subcommands):${plain}                       │
│                                                                │
│  ${blue}xpanel${plain}                       - Admin Management Script          │
│  ${blue}xpanel start${plain}                 - Start                            │
│  ${blue}xpanel stop${plain}                  - Stop                             │
│  ${blue}xpanel restart${plain}               - Restart                          │
|  ${blue}xpanel restart-xray${plain}          - Restart Xray                     │
│  ${blue}xpanel status${plain}                - Current Status                   │
│  ${blue}xpanel settings${plain}              - Current Settings                 │
│  ${blue}xpanel enable${plain}                - Enable Autostart on OS Startup   │
│  ${blue}xpanel disable${plain}               - Disable Autostart on OS Startup  │
│  ${blue}xpanel log${plain}                   - Check logs                       │
│  ${blue}xpanel banlog${plain}                - Check Fail2ban ban logs          │
│  ${blue}xpanel update${plain}                - Update                           │
│  ${blue}xpanel update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}xpanel legacy${plain}                - Legacy version                   │
│  ${blue}xpanel install${plain}               - Install                          │
│  ${blue}xpanel uninstall${plain}             - Uninstall                        │
└────────────────────────────────────────────────────────────────┘"
}
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}XPanel Panel Management Script${plain}                │
